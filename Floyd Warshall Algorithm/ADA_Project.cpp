...
void printSolution(int cost[N][N], int path[N][N])
{
	...
	for (int v = 0; v < N; v++)
	{
		for (int u = 0; u < N; u++)
		{
			if (u != v && path[v][u] != -1)
			{
				cout << "Shortest Path from HOUSE " << v <<
					 " to HOUSE " << u << " is (" << v << "->";
				printPath(path, v, u);
				cout << u << ")" << endl;
			}
...
int main()
{
	...
	int adjMatrix[N][N] ;
	cout<<"Enter the adjacency matrix of dimensions : "<<N<<"x"<<N<<"(Note : I can vary the dimensions (Houses/Vertex) inside the code by just varying the value of one variable )"<<endl ;
	cout<<"\n\n\n" ;
	for(int i=0 ; i<N ; i++)
    {
        for(int j=0 ; j<N;j++)
